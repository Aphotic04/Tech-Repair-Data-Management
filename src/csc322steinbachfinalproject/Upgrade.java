...
//Upgrade class
public class Upgrade extends Item implements Serializable{
    //Member variables
    private final double COMPUTER_UPGRADE = 50;
    private final double LAPTOP_UPGRADE = 70;
    private final double CONTROLLER_UPGRADE = 40;
    private transient StringProperty itemSubType;
    private final String itemType = "Upgrade";
    ...
    void setPrices(String itemType, double tempPrice, double tempHourly) {
        ...
        switch (getItemSubType().toLowerCase()) {
            case "computer" -> setPrice(COMPUTER_UPGRADE);
            case "laptop" -> setPrice(LAPTOP_UPGRADE);
            case "controller" -> setPrice(CONTROLLER_UPGRADE);
            default -> {
                setPrice(tempPrice);
