...
//Misc class
public class Misc extends Item implements Serializable{
    //Member variables
    private final double LOCKED_ACCESS = 150;
    private final double SET_UP = 80;
    private final double SET_UP_TRANSFER = 50;
    private final double LAPTOP_SCREEN_REPLACEMENT = 120;
    private final double MOBIILE_SCREEN_REPLACEMENT = 150;
    private final double HARD_DRIVE_RECOVERY = 120;
    private transient StringProperty itemSubType;
    private final String itemType = "Misc";
    ...
    void setPrices(String itemType, double tempPrice, double tempHourly) {
        ...
        switch (getItemSubType().toLowerCase()) {
            case "locked access" -> setPrice(LOCKED_ACCESS);
            case "set up" -> setPrice(SET_UP);
            case "set up transfer" -> setPrice(SET_UP_TRANSFER);
            case "laptop screen replacement" -> setPrice(LAPTOP_SCREEN_REPLACEMENT);
            case "mobile screen replacement" -> setPrice(MOBIILE_SCREEN_REPLACEMENT);
            case "hard drive recovery" -> setPrice(HARD_DRIVE_RECOVERY);
            default -> {
                setPrice(tempPrice);
